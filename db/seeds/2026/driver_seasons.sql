INSERT INTO driver_seasons (driver_id, season, constructor_id, rookie, price) VALUES

/* ========= INVICTA RACING ========= */
(1,  2026, 1, 1, 24.0),  -- Rafael Câmara
(2,  2026, 1, 0, 20.0),  -- Joshua Dürksen

/* ========= HITECH TGR ========= */
(3,  2026, 2, 0, 14.0),  -- Ritomo Miyata
(4,  2026, 2, 1, 15.0),  -- Colton Herta

/* ========= CAMPOS RACING ========= */
(5,  2026, 3, 1, 15.0),  -- Noel León
(6,  2026, 3, 1, 20.0),  -- Nikola Tsolov

/* ========= DAMS LUCAS OIL ========= */
(7,  2026, 4, 0, 20.0),  -- Dino Beganovic
(8,  2026, 4, 1, 14.0),  -- Roman Bilinski

/* ========= MP MOTORSPORT ========= */
(9,  2026, 5, 0, 15.0),  -- Gabriele Minì
(10, 2026, 5, 0, 15.0),  -- Oliver Goethe

/* ========= PREMA RACING ========= */
(11, 2026, 6, 0, 15.0),  -- Sebastián Montoya
(12, 2026, 6, 1, 16.0),  -- Mari Boya

/* ========= RODIN MOTORSPORT ========= */
(13, 2026, 7, 1, 18.0),  -- Martinius Stenshorne
(14, 2026, 7, 0, 22.0),  -- Alexander Dunne

/* ========= ART GRAND PRIX ========= */
(15, 2026, 8, 0, 9.0),   -- Kush Maini
(16, 2026, 8, 1, 12.0),  -- Tasanapol Inthraphuvasak

/* ========= AIX RACING ========= */
(17, 2026, 9, 1, 7.0),   -- Emerson Fittipaldi Jr.
(18, 2026, 9, 0, 5.0),   -- Cian Shields

/* ========= VAN AMERSFOORT RACING ========= */
(19, 2026, 10, 1, 7.0),  -- Nicolás Varrone
(20, 2026, 10, 1, 10.0),  -- Rafael Villagómez

/* ========= TRIDENT ========= */
(21, 2026, 11, 1, 10.0), -- Laurens van Hoepen
(22, 2026, 11, 0, 7.0)   -- John Bennett
ON CONFLICT (driver_id, season) DO NOTHING;