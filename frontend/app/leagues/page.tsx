      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {leagues.length > 0 ? (
                ...
                <div className="relative space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right">
                      <div className="text-xs text-slate-400">{t("card.code")}</div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-black/20 border border-slate-800/50 rounded-lg p-3 text-center">
                      <p className="text-[11px] text-slate-400 mb-1">{t("card.members")}</p>
                      <p className="text-2xl font-bold text-white">{league.member_count}</p>
                    </div>
                    <div className="bg-black/20 border border-slate-800/50 rounded-lg p-3 text-center">
                      <p className="text-[11px] text-slate-400 mb-1">{t("card.position")}</p>
                      <p className="text-2xl font-bold text-white">#{league.rank}</p>
                    </div>
                    <div className="bg-black/20 border border-slate-800/50 rounded-lg p-3 text-center">
                      <p className="text-[11px] text-slate-400 mb-1">{t("card.points")}</p>
          ...
          <div className="text-center py-16">
            <p className="text-slate-300 mb-4">{t("empty.description")}</p>
            <button
              onClick={() => setShowCreateModal(true)}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="relative bg-gradient-to-br from-slate-900/90 to-slate-900/60 border border-slate-800/50 rounded-2xl p-8 max-w-md w-full">
            ...
            <div className="relative">
              <h2 className="text-2xl font-bold text-white mb-2">{t("createModal.title")}</h2>
              <p className="text-slate-300 mb-6">{t("createModal.description")}</p>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="relative bg-gradient-to-br from-slate-900/90 to-slate-900/60 border border-slate-800/50 rounded-2xl p-8 max-w-md w-full">
            ...
            <div className="relative">
              <h2 className="text-2xl font-bold text-white mb-2">{t("joinModal.title")}</h2>
              <p className="text-slate-300 mb-6">{t("joinModal.description")}</p>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 h-11 rounded-lg bg-gradient-to-r from-accent to-accent/80 text-white font-semibold hover:opacity-95 disabled:opacity-50"
                >
                  {joining ? t("joinModal.submitting") : t("joinModal.submit")}
                </button>
                <button
                  onClick={() => {
                    setShowJoinModal(false);
