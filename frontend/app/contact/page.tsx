...
export default function ContactPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error ?? t("form.submitting"));
      }
      ...
      setError(err?.message ?? t("form.submitting"));
    } finally {
      setLoading(false);
    }
  }
  ...
  function getPlaceholder(subject: string): string {
    switch (subject) {
      case "bug": return t("form.message.placeholders.bug");
      case "improvement": return t("form.message.placeholders.improvement");
      case "feedback": return t("form.message.placeholders.feedback");
      case "question": return t("form.message.placeholders.question");
      default: return t("form.message.placeholders.default");
    }
  }
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <SubjectButton active={subject === "improvement"} onClick={() => setSubject("improvement")}
                      icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>}
                      label={t("form.subjects.improvement")}
                    />
                    <SubjectButton active={subject === "bug"} onClick={() => setSubject("bug")}
                      icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>}
                      label={t("form.subjects.bug")}
                    />
                    <SubjectButton active={subject === "feedback"} onClick={() => setSubject("feedback")}
                      ...
                      label={t("form.subjects.feedback")}
                    />
                    <SubjectButton active={subject === "question"} onClick={() => setSubject("question")}
                      ...
                      label={t("form.subjects.question")}
                    />
                  </div>
                </div>
                ...
                {/* Prénom */}
                <div>
                  <label className="block text-sm font-medium text-white mb-2">{t("form.firstName.label")}</label>
                  <input
                    value={firstName}
                ...
                {/* Email */}
                <div>
                  <label className="block text-sm font-medium text-white mb-2">{t("form.email.label")}</label>
                  <input
                    type="email"
                    value={email}
                ...
                {/* Message */}
                <div>
                  <label className="block text-sm font-medium text-white mb-2">{t("form.message.label")}</label>
                  <textarea
                    value={message}
                  ...
                  <div className="mt-3 p-3 bg-amber-950/20 border border-amber-900/30 rounded-lg">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-xs text-amber-300/90">
                        {subject === "bug" ? t("form.message.tipBug") : t("form.message.tipDefault")}
                      </p>
                    </div>
                  </div>
                </div>
...
function SubjectButton({ active, onClick, icon, label }: SubjectButtonProps) {
  return (
    <button
      ...
      `}
    >
      <div className={active ? "text-red-400" : "text-slate-500"}>{icon}</div>
...
function InfoCard({ icon, title, description, link, linkLabel }: InfoCardProps): React.JSX.Element {
  const content = (
