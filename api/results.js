...
router.get("/weekend/:weekendId", (req, res) => {
  ...
  try {
    ...
    const results = db.prepare(`
      ...
      LEFT JOIN qualifying_results q ON q.driver_id = d.id AND q.race_weekend_id = ?
      LEFT JOIN sprint_results s ON s.driver_id = d.id AND s.race_weekend_id = ?
      LEFT JOIN feature_results f ON f.driver_id = d.id AND f.race_weekend_id = ?
      WHERE wp.race_weekend_id = ?
      ORDER BY COALESCE(f.finish_position, 99), COALESCE(s.finish_position, 99)
    `).all(weekendId, weekendId, weekendId, weekendId);
